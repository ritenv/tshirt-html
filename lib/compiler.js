...
Compiler.prototype = {
	...
	compile: function(localRaw) {
        ...
        var doScan = function(content) {
            var blocks = Scanner.scan(raw);
            var compiledContent = "";
            if (blocks.length) {
                for (var i = blocks.length - 1; i >= 0; i--) {
                    var tag = blocks[i].tag;
                    var content = blocks[i].block;
                    compiledContent += pInstance.wrapTags(tag, doScan(content));
                };
            }
            return compiledContent;
        };
        console.log(doScan(localRaw));
	...
	wrapTags: function(tag, content, attribs) {
		...
		//get indent
		var indentMatch = tag.match(pInstance.regexps('extractIndent'));
		var indent = indentMatch ? indentMatch.pop() : '';
		var attribs = attribs ? ' ' + attribs : '';
		tag = tag.trim();
		return indent + "<" + tag + attribs + ">" + content + "</" + tag + ">";
	},
